...
deny contains "production migration execution is prohibited" if input.environment == "prod" and input.action == "migration.execute"
deny contains "production messaging mutations are prohibited" if input.environment == "prod" and input.action_class == "messaging-mutation"
deny contains "artifact signing and production publication are outside autonomous scope" if input.action in {"artifact.sign", "artifact.publish.production"}
deny contains "production deployment is outside autonomous scope" if input.action == "deployment.production"
...
deny contains "unapproved Maven repository" if input.action == "dependency.download" and not input.target in input.authorization.package_repositories
...
action_needs_approval if input.action in {
  ...
  "dependency.add",
  "dependency.upgrade",
  "build.parent-or-bom.update",
  "database.write.nonprod",
  "messaging.publish.nonprod",
  "messaging.offset.modify.nonprod",
  "migration.execute.nonprod",
  "deployment.request.nonprod"
}
